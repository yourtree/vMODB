...
import dk.ku.di.dms.vms.modb.storage.infra.MemoryManagerBuilder;
...
public class OrderedRecordBuffer {
    ...
    // entry size :
    private final int entrySize = 1 + Integer.BYTES + (3 * Long.BYTES);
    ...
    private final int deltaKey = 1 + Long.BYTES;
    private final int deltaOffset = 1 + Long.BYTES + Integer.BYTES;
    private final int deltaNext = 1 + Integer.BYTES + (2 * Long.BYTES);
    private final int deltaPrevious = 1;
    ...
    private void reorder(){
        ...
        MemoryManager.MemoryClaimed mc = MemoryManagerBuilder.build().claim(entrySize);
