...
Java_dk_ku_di_dms_vms_modb_iouring_IoUring_createCqes(JNIEnv *env, jclass cls, jint count) {
...
JNIEXPORT jlong JNICALL
Java_dk_ku_di_dms_vms_modb_iouring_IoUring_create(JNIEnv *env, jclass cls, jint maxEvents) {
...
Java_dk_ku_di_dms_vms_modb_iouring_IoUring_close(JNIEnv *env, jclass cls, jlong ring_address) {
    struct io_uring *ring = (struct io_uring *) ring_address;
    io_uring_queue_exit(ring);
}

JNIEXPORT jint JNICALL
Java_dk_ku_di_dms_vms_modb_iouring_IoUring_submitAndGetCqes(JNIEnv *env, jclass cls, jlong ring_address, jobject byte_buffer, jlong cqes_address, jint cqes_size, jboolean should_wait) {
    struct io_uring *ring = (struct io_uring *) ring_address;
...
JNIEXPORT void JNICALL
Java_dk_ku_di_dms_vms_modb_iouring_IoUring_freeCqes(JNIEnv *env, jclass cls, jlong cqes_address) {
    struct io_uring_cqe **cqes = (struct io_uring_cqe **) cqes_address;
    free(cqes);
}

JNIEXPORT jstring JNICALL
Java_dk_ku_di_dms_vms_modb_iouring_IoUring_getCqeIpAddress(JNIEnv *env, jclass cls, jlong cqes_address, jint cqe_index) {
    struct io_uring_cqe **cqes = (struct io_uring_cqe **) cqes_address;
    struct io_uring_cqe *cqe = cqes[cqe_index];
...
JNIEXPORT void JNICALL
Java_dk_ku_di_dms_vms_modb_iouring_IoUring_markCqeSeen(JNIEnv *env, jclass cls, jlong ring_address, jlong cqes_address, jint cqe_index) {
    struct io_uring *ring = (struct io_uring *) ring_address;
    struct io_uring_cqe **cqes = (struct io_uring_cqe **) cqes_address;
    
    // Strict safety checks
    if (ring == NULL || cqes == NULL) {
        // Log warning but don't throw exception during cleanup
        fprintf(stderr, "markCqeSeen: null pointer (ring=%p, cqes=%p)\n", ring, cqes);
        return;
    }
    
    // Validate cqe_index bounds (assuming reasonable maximum)
    if (cqe_index < 0 || cqe_index >= 1024) {  // 1024 is a reasonable max
        fprintf(stderr, "markCqeSeen: invalid cqe_index %d\n", cqe_index);
        return;
    }
    
    struct io_uring_cqe *cqe = cqes[cqe_index];
    if (cqe == NULL) {
        // This can happen legitimately during shutdown, don't throw exception
        fprintf(stderr, "markCqeSeen: null cqe at index %d\n", cqe_index);
        return;
    }
    
    // Extract and clear user_data BEFORE calling io_uring_cqe_seen
    // This prevents race conditions where the CQE might be reused
    void *user_data = (void *) cqe->user_data;
    cqe->user_data = 0;
    
    // Mark CQE as seen - after this point, the CQE may be reused by io_uring
    io_uring_cqe_seen(ring, cqe);
    
    // Now safely free the request data we extracted earlier
    if (user_data != NULL) {
        free(user_data);
    }
}
...
JNIEXPORT void JNICALL
Java_dk_ku_di_dms_vms_modb_iouring_IoUring_queueAccept(JNIEnv *env, jclass cls, jlong ring_address, jint server_socket_fd) {
    struct io_uring *ring = (struct io_uring *) ring_address;
...
JNIEXPORT void JNICALL
Java_dk_ku_di_dms_vms_modb_iouring_IoUring_queueConnect(JNIEnv *env, jclass cls, jlong ring_address, jint socket_fd, jstring ip_address, jint port) {
    struct io_uring *ring = (struct io_uring *) ring_address;
...
JNIEXPORT jlong JNICALL
Java_dk_ku_di_dms_vms_modb_iouring_IoUring_queueRead(JNIEnv *env, jclass cls, jlong ring_address, jint fd, jobject byte_buffer, jint buffer_pos, jint buffer_len, jlong io_offset) {
...
JNIEXPORT jlong JNICALL
Java_dk_ku_di_dms_vms_modb_iouring_IoUring_queueWrite(JNIEnv *env, jclass cls, jlong ring_address, jint fd, jobject byte_buffer, jint buffer_pos, jint buffer_len, jlong io_offset) {
    void *buffer = (*env)->GetDirectBufferAddress(env, byte_buffer);
    if (buffer == NULL) {
...
JNIEXPORT void JNICALL
Java_dk_ku_di_dms_vms_modb_iouring_IoUring_queueClose(JNIEnv *env, jclass cls, jlong ring_address, jint fd) {
    struct io_uring *ring = (struct io_uring *) ring_address;
    if (io_uring_sq_space_left(ring) <= 1) {
        throw_exception(env, "io_uring_sq_space_left", -EBUSY);
        return;
...
JNIEXPORT void JNICALL
Java_dk_ku_di_dms_vms_modb_iouring_AbstractIoUringChannel_close(JNIEnv *env, jclass cls, jint fd) {
    shutdown(fd, SHUT_WR);
    close(fd);
}

JNIEXPORT void JNICALL
Java_dk_ku_di_dms_vms_modb_iouring_IoUring_queueFsync(JNIEnv *env, jclass cls, jlong ring_address, jint fd, jboolean datasync) {
    struct io_uring *ring = (struct io_uring *) ring_address;
    if (io_uring_sq_space_left(ring) <= 1) {
        throw_exception(env, "io_uring_sq_space_left", -EBUSY);
        return;
    ...
    req->event_type = EVENT_TYPE_FSYNC;
    req->fd = fd;
